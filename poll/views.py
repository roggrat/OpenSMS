...
def responses_as_csv(req, pk):
    ...
    response['Content-Disposition'] = 'attachment; filename=OpenSMS-export.csv'
...
def activity_as_csv(req):
    """ CSV export of the Message activity log"""
    activity = Message.objects.all().order_by('-pk')

    # Create the HttpResponse object with the appropriate CSV header.
    response = HttpResponse(mimetype='text/csv')
    response['Content-Disposition'] = 'attachment; filename=OpenSMS-activity.csv'
